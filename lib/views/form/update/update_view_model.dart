...
class UpdateViewModel extends BaseViewModel {
  ...
  String get wfId => this._wfId;
  set wfId(String value) {
    this._wfId = value;
    notifyListeners();
  }
  ...
  init(String datastoreId, String itemId, String wfId) async {
    try {
      ...
      if (wfId != null && wfId.isNotEmpty) {
        this.wfId = wfId;
        var wk = await ApiService.getWorkflow(wfId);
        var fs = wk.params.fields;
        if (fs.isNotEmpty) {
          fieldList.addAll(fs.split(','));
        }

        if (fieldList.length > 0) {
          fieldList.forEach((f) {
            var field = _fs.singleWhere(
              (it) => it.fieldId == f,
              orElse: () => null,
            );
            if (field != null) {
        ...
        var wfs = await ApiService.getUserWorkflows(datastoreId, action: 'update');
      ...
      for (var field in this.fields) {
        ...
        if (field.fieldType == 'text' || field.fieldType == 'textarea') {
          // 将原有的值存入旧的文件中
          _oldMap[field.fieldId] = Common.getValue(items.items[field.fieldId], dataType: field.fieldType);
  ...
  Future<void> submit(Map<String, Object> valueMap) async {
    ...
    if (this.wfId.isNotEmpty) {
      bool result = await ApiService.updateItem(
        datastoreId,
        itemId,
        items,
        wfId: this.wfId,
      ...
      bool result = await ApiService.updateItem(datastoreId, itemId, items);
      if (result) {
        // BotToast.showText(text: 'info_update_success'.tr);
        eventBus.fire(RefreshEvent());
        _nav.pop();
  ...
  uniqueValidator(List<Field> uniqueFields) async {
    ...
    uniqueFields.forEach((u) {
      // 旧值
      var oldVal = this._oldMap[u.fieldId];
      // 新值
      var newVal = this.form.controls[u.fieldId];

      if (oldVal != null && newVal != null && oldVal == newVal.value) {
        eqCount++;
      }
